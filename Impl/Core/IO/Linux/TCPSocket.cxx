...
#include "SimpleTUI/Core/IO/Linux/TCPSocket.h"
...
hostent* tcp_socket::host(const char* node, uint port, sockaddr_in* _addr_in, std::string& NodeIP)
{
    ...
    if (!he ) {
        AppBook::Error() << "gethostbyname failed!!";
...
int tcp_socket::mkaddr(void* addr, int* addr_len, const char* addr_str, const char* proto)
{
    ...
    StrBreak::ConfigData Data = {saddress,":", StrBreak::Keep};
    StrBreak Brk;
    auto n = Brk(Data);
    if (n!=2) return -1;
    std::string inaddress = (*Data.Words.begin())();
    std::string serviceport = Data.Words.back()();
    std::string protoc = proto;
    auto* ap = (sockaddr_in*)addr;
    hostent* hp = 0;
    servent* sp = 0;
    char *cp;
    long lv;
    ...
    *addr_len = sizeof(*ap);
    ...
    AppBook::Info() << "finalized socket addr infos [port]::" << ntohs(ap->sin_port);
    return 0;
}
